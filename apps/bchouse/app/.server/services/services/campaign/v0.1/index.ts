...
export class CampaignContract extends AbstractCampaignContract {
  constructor(
  ...
  override async complete(
    ...
    if (campaignUtxo) {
      ...
      completedTx = await builder.send()
    }
  ...
  override async forwardPledge(
    ...
    if (pledgeType === 'STARTED' && campaignUtxo?.categoryId) {
      ...
      if (pledgeUtxo.vout !== 0) {
        ...
        await pledgeRepository.saveForwardedPledge({
          pledgeRequestId: pledgeId,
          satoshis: result.pledgeUtxo.satoshis,
          txid: result.pledgeUtxo.txid,
          vout: result.pledgeUtxo.vout,
        })
