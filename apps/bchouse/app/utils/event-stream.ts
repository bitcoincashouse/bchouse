...
export function eventStream(
  ...
  if (headers.has('Content-Type')) {
    logger.warn('Overriding Content-Type header to `text/event-stream`')
  }

  if (headers.has('Cache-Control')) {
    logger.warn('Overriding Cache-Control header to `no-cache`')
  }

  if (headers.has('Connection')) {
    logger.warn('Overriding Connection header to `keep-alive`')
  }
