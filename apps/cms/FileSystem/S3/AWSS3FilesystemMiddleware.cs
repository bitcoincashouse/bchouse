...
using UmbracoHosting = Umbraco.Cms.Core.Hosting;
...
namespace Our.Umbraco.StorageProviders.AWSS3
{
  public class AWSS3FileSystemMiddleware : IMiddleware
  {
    ...
    /// 
    public AWSS3FileSystemMiddleware(IOptionsMonitor<AWSS3FileSystemOptions> options, IAWSS3FileSystemProvider fileSystemProvider, UmbracoHosting.IHostingEnvironment hostingEnvironment, IAmazonS3 s3Client)
    ...
    protected AWSS3FileSystemMiddleware(string name, IOptionsMonitor<AWSS3FileSystemOptions> options, IAWSS3FileSystemProvider fileSystemProvider, UmbracoHosting.IHostingEnvironment hostingEnvironment, IAmazonS3 s3Client)
    {
    ...
    private async Task HandleRequestAsync(HttpContext context, RequestDelegate next)
    {
      ...
      if (!context.Request.Path.StartsWithSegments(_rootPath, StringComparison.InvariantCultureIgnoreCase) || request.Path.Value is null)
      {
    ...
    private static GetObjectMetadataRequest GetAccessCondition(HttpRequest request, string key, string bucketName)
    {
      ...
      if (string.IsNullOrEmpty(range))
      {
        ...
        if (DateTime.TryParse(request.Headers["If-Modified-Since"], CultureInfo.InvariantCulture, out var ifModifiedSince))
        {
          getObjectMetadataRequest.ModifiedSinceDateUtc = ifModifiedSince;
          return getObjectMetadataRequest;
        }
      }
      else
      {
        ...
        if (DateTime.TryParse(request.Headers["If-Unmodified-Since"], CultureInfo.InvariantCulture, out var ifUnmodifiedSince))
        {
          getObjectMetadataRequest.UnmodifiedSinceDateUtc = ifUnmodifiedSince;
          return getObjectMetadataRequest;
        }
      }
    ...
    private void OptionsOnChange(AWSS3FileSystemOptions options, string? name, UmbracoHosting.IHostingEnvironment hostingEnvironment)
    {
